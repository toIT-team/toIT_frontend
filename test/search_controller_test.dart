...
import 'package:toit/controllers/search_controller.dart';
import 'package:toit/models/dto/search_response_dto.dart';
import 'package:toit/models/search/search_result_item.dart';
import 'package:toit/services/search_api_client.dart';
...
class _StubSearchApi extends SearchApiClient {
  ...
  Future<SearchResponseDto> search({required String keyword}) async {
    return const SearchResponseDto(
      folders: [SearchFolderItemDto(foldersId: 1, name: '보관함 A')],
      schedules: [
...
void main() {
  ...
  test('동일 필터 재선택 시 해제되고, 보관함 필터는 폴더 결과만 남긴다', () async {
